...
public fun PsiReference.canBeReferenceTo(candidateTarget: PsiElement): Boolean {
    // optimization
    if (getElement().getContainingFile() == candidateTarget.getContainingFile()) return true
    return ProjectRootsUtil.isInProjectOrLibSource(getElement())
           && (ProjectRootsUtil.isInProjectOrLibSource(candidateTarget)
               || !candidateTarget.getContainingFile().getViewProvider().isPhysical())
}
