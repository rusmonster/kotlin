...
public class KotlinCoreEnvironment private(
    ...
    private fun fillClasspath(configuration: CompilerConfiguration) {
        for (root in configuration.getList(CommonConfigurationKeys.CONTENT_ROOTS)) {
            val javaRoot = root as? JavaContentRoot ?: continue
    ...
    private fun contentRootToVirtualFile(root: JavaContentRoot): VirtualFile? {
        when (root) {
            is JvmClasspathRoot -> {
                return if (root.file.isFile()) findJarRoot(root) else findLocalFile(root)
            }
            is JavaSourceRoot -> {
                return if (root.file.isDirectory()) findLocalFile(root) else null
            }
            else -> return null
        }
    }

    private fun findLocalFile(root: JavaContentRoot): VirtualFile? {
        val path = root.file
